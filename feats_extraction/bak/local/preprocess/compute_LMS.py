...
import glob
from logpowerspec import logmagspec
...
def _process_utterance(out_dir, utt_idx, wav_path, stft_conf):
    lms = logmagspec(wav_path, sr=stft_conf['sample_rate'], n_fft=stft_conf['n_fft'], 
            hop_length=stft_conf['hop_length'], win_length=stft_conf['win_length'], window=stft_conf['window'])
    lms_filename = os.path.join(out_dir, utt_idx+".npy")
    np.save(lms_filename, lms.astype(np.float32), allow_pickle=False)
    return lms_filename
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_dir', type=str, default='./_feats/LPS')
